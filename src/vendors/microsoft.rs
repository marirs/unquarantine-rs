use crate::{
    Result,
    error::Error,
    utils::{self, bytearray_xor, rc4_decrypt, unpack_i32},
};

const KEY: &[i32] = &[
    ,
];

/// Microsoft Defender PC - partially supported (D3 45 C5 99 header)
pub fn pc_unquarantine(data: &[u8]) -> Result<Vec<Vec<u8>>> {
    let fsize = data.len();
    if fsize < 12 || data[0] != 0x0B || data[1] != 0xAD || data[2] != 0x00 {
        return Err(Error::CannotUnQuarantineFile("mse".to_string()));
    }
    let mut sbox = ksa();
    let outdata = rc4_decrypt(&mut sbox, data);

    let headerlen = unpack_i32(utils::slice(&outdata, 8, 4, "mse headerlen")?)? as i64 + 0x28;
    if !(12..=fsize as i64).contains(&headerlen) {
        return Err(Error::Invalid("mse headerlen"));
    }
    let headerlen = headerlen as usize;
    let origlen =
        unpack_i32(utils::slice(&outdata, headerlen - 12, 4, "mse origlen")?)? as u32 as usize;

    if origlen.checked_add(headerlen) != Some(fsize) {
        return Err(Error::CannotUnQuarantineFile("mse".to_string()));
    }

    Ok(vec![utils::tail(&outdata, headerlen, "mse body")?.to_vec()])
}

/// Microsoft Defender MAC
pub fn mac_unquarantine(data: &[u8]) -> Result<Vec<Vec<u8>>> {
    Ok(vec![bytearray_xor(data.to_owned(), 0x25)])
}

/// Microsoft Antimalware / Microsoft Security Essentials
pub fn antimalware_unquarantine(data: &[u8]) -> Result<Vec<Vec<u8>>> {
    Ok(vec![bytearray_xor(data.to_owned(), 0xff)])
}

fn ksa() -> Vec<u8> {
    let mut sbox: Vec<u8> = (0..=255).collect();
    let mut j = 0_usize;
    for i in 0..256 {
        j = (j + sbox[i] as usize + KEY[i] as usize) % 256;
        sbox.swap(i, j)
    }
    sbox
}
